...
export default function useRationale() {
  ...
  const [hasAutoGenerated, setHasAutoGenerated] = useState(false) // Track if we've auto-generated
  ...
  const generateAIRationaleForCompany = useCallback(async (filters = {}) => {
    try {
  ...
  const resetAutoGeneration = () => {
    setHasAutoGenerated(false)
  }

  const refreshRationales = async () => {
    await fetchRationales()
  }

  useEffect(() => {
    // First, try to fetch existing rationales
    fetchRationales()
    
    // Then automatically trigger AI analysis if no recent rationale exists
    const autoGenerateRationale = async () => {
      try {
        console.log('🤖 Checking if auto-generation is needed...')
        console.log('Current state:', { hasAutoGenerated, rationalesCount: rationales.length })
        
        // Only auto-generate once per session and if we don't have recent data
        if (hasAutoGenerated) {
          console.log('🤖 Auto-generation already happened this session')
          return
        }
        
        // Check if we have recent rationales (within last 24 hours)
        const hasRecentRationale = rationales.some(rationale => {
          if (!rationale.created_at) return false
          const createdAt = new Date(rationale.created_at)
          const now = new Date()
          const hoursDiff = (now - createdAt) / (1000 * 60 * 60)
          console.log(`Rationale ${rationale.id} created ${hoursDiff.toFixed(1)} hours ago`)
          return hoursDiff < 24
        })
        
        console.log('Has recent rationale:', hasRecentRationale)
        
        // If no recent rationale, automatically generate one
        if (!hasRecentRationale && rationales.length === 0) {
          console.log('🤖 Auto-triggering AI rationale generation...')
          await generateAIRationaleForCompany()
          setHasAutoGenerated(true) // Mark that auto-generation happened
        } else if (hasRecentRationale) {
          console.log('✅ Recent rationale found, no need for auto-generation')
          setHasAutoGenerated(true) // Mark as handled
        } else {
          console.log('📊 Rationales exist but none are recent, allowing manual generation')
          setHasAutoGenerated(true) // Mark as handled
        }
      } catch (error) {
        console.warn('Auto-generation failed, will show manual button:', error)
        setHasAutoGenerated(true) // Mark as handled to prevent retries
      }
    }
    
    // Wait a bit for initial data to load, then auto-generate if needed
    const timer = setTimeout(autoGenerateRationale, 1000)
    
    return () => clearTimeout(timer)
  }, [companyId, rationales.length, generateAIRationaleForCompany, hasAutoGenerated])
